...
(async () => {
  const { GIT_COMMITTER_EMAIL, GIT_COMMITTER_NAME, GITHUB_TOKEN, NPM_TOKEN } =
    process.env;

  if (!GITHUB_TOKEN || !GIT_COMMITTER_NAME || !GIT_COMMITTER_EMAIL) {
    throw new Error(
      'GIT_COMMITTER_EMAIL, GITHUB_TOKEN and GIT_COMMITTER_NAME must be set',
    );
  }

  const repoAuthedUrl = `https://${GITHUB_TOKEN}@github.com/Growth-UI/Growth-UI-React`;
  const repoPublicUrl = 'https://github.com/Growth-UI/Growth-UI-React';
  ...
  const semanticChanges = newCommits.reduce(
    (acc, { subj, body, short, hash }) => {
      semanticRules.forEach(({ group, releaseType, prefixes, keywords }) => {
        ...
        const change =
          subj.match(prefixMatcher)?.[0] || body.match(keywordsMatcher)?.[2];
...
${commits.join('\n')}`,
  ...
  await $`yarn lerna version ${nextReleaseType} --no-push --no-git-tag-version --no-changelog --yes -m "chore(release): version [skip ci]"`;
  ...
  if (lastTag !== nextVersion) {
    ...
    await $`npm publish --no-git-tag-version --access public ./packages/growth-ui-react/dist`;
    console.log(
      '=================================== Done ===================================',
    );
  }
