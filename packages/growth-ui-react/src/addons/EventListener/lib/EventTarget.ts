...
import { EventListeners, TargetElement, CallableEventListener } from '../types';

export default class EventTarget {
  private readonly handlers: Map<string, CallableEventListener> = new Map();
  ...
  private removeTargetHandler(eventType: string) {
    if (this.handlers.has(eventType)) {
      this.target.removeEventListener(eventType, this.handlers.get(eventType) as EventListener, true);
